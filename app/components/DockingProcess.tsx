...
export function DockingProcess({
  loadingStage,
  targetName,
  pdbId,
  patientId,
  ...
  const [logs, setLogs] = useState<LogItem[]>([
    {
      id: 0,
      text: "pipeline started — 4 agents online",
      cls: "fetch",
      tag: "fetch",
    },
  ]);
  ...
  useEffect(() => {
    if (loadingStage >= 0 && !flags.current.f0) {
      ...
      setTimeout(
        () => pushLog("extracting ligand coordinates", "fetch", "fetch"),
        500,
      );
    }
    ...
    if (loadingStage >= 4 && !flags.current.f4) {
      ...
      pushLog(
        "final ΔG = −10.40 kcal/mol — very high affinity",
        "final",
        "final",
      );
    }
  ...
  function draw(progress: number, t: number) {
    ...
    // receptor lock cavity (Biological Lock: Protein active site with custom binding groove)
    ctx.save();
    ctx.strokeStyle = "#0284c7";
    ctx.lineWidth = 4;
    ctx.fillStyle = "rgba(224, 242, 254, 0.7)";
    ctx.beginPath();
    // Precise Lock Shape with cutout receptacle slots matching key teeth
    ctx.moveTo(cx - 100, cy - 60);
    ctx.lineTo(cx - 30, cy - 60);
    ctx.lineTo(cx - 30, cy - 90); // Top recess slot for key tooth 1
    ctx.lineTo(cx + 10, cy - 90);
    ctx.lineTo(cx + 10, cy - 60);
    ctx.lineTo(cx + 80, cy - 60);
    ctx.quadraticCurveTo(cx + 110, cy - 60, cx + 110, cy - 30);
    ctx.lineTo(cx + 110, cy + 10);
    ctx.lineTo(cx + 140, cy + 10); // Right recess slot for key tooth 3 tip
    ctx.lineTo(cx + 140, cy + 40);
    ctx.lineTo(cx + 110, cy + 40);
    ctx.lineTo(cx + 110, cy + 60);
    ctx.quadraticCurveTo(cx + 110, cy + 90, cx + 80, cy + 90);
    ctx.lineTo(cx - 30, cy + 90);
    ctx.lineTo(cx - 30, cy + 120); // Bottom recess slot for key tooth 2
    ctx.lineTo(cx + 10, cy + 120);
    ctx.lineTo(cx + 10, cy + 90);
    ctx.lineTo(cx - 100, cy + 90);
    ctx.quadraticCurveTo(cx - 130, cy + 90, cx - 130, cy + 60);
    ctx.lineTo(cx - 130, cy - 30);
    ctx.quadraticCurveTo(cx - 130, cy - 60, cx - 100, cy - 60);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();

    // Receptor interior binding pockets / catalytic residues (Lock teeth receptacles)
    const receptorGrooves = [
      { x: cx - 10, y: cy - 90, label: "ASP128" },
      { x: cx - 10, y: cy + 120, label: "SER195" },
      { x: cx + 140, y: cy + 25, label: "HIS57" },
      { x: cx - 130, y: cy, label: "GLU199" },
    ];
    receptorGrooves.forEach((res) => {
      ctx.fillStyle = "#0369a1";
      ctx.beginPath();
      ctx.arc(res.x, res.y, 14, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#ffffff";
      ctx.font = "bold 9px system-ui, sans-serif";
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(res.label, res.x, res.y);
    });
    ctx.restore();

    // ligand path per phase (Lock & Key: Drug key travels from top-left into the receptor lock cavity center)
    let lx = cx,
      ly = cy;
    if (phase === 0) {
      ...
      // Smooth interpolation right into the exact lock center (cx, cy)
      const e = raw < 0.5 ? 2 * raw * raw : 1 - Math.pow(-2 * raw + 2, 2) / 2;
      lx = W * 0.2 + 200 + (cx - (W * 0.2 + 200)) * e;
      ly = H * 0.2 + 200 + (cy - (H * 0.2 + 200)) * e;
      if (phase === 2 && raw < 0.7) {
        // adjustment wobble as key enters lock
        const j = (1 - raw) * 8;
        lx += Math.sin(t * 20) * j;
        ly += Math.cos(t * 18) * j;
      }
    ...
    // Hydrogen bonds / electrostatic lock-in when key is inside lock
    if ((phase === 2 || phase === 3) && progress > 0.75) {
      ctx.save();
      ...
      ctx.shadowBlur = 12;
      // Connect key protrusions to receptor residues
      ctx.beginPath();
      ctx.moveTo(lx - 45, ly);
      ctx.lineTo(cx - 110, cy - 40);
      ctx.stroke();
    ...
    // Drug Molecule shaped precisely as a "Key" (Complementary spatial geometry with protruding molecular functional groups)
    ctx.save();
    ctx.translate(lx, ly);
    if (phase >= 2 && progress > 0.8) {
      // settled snug in lock
      ctx.rotate(0);
    } else {
      ctx.rotate(t * 3);
    }

    // Key Bow (Main drug scaffold / ring core - Left side)
    ctx.fillStyle = "#f59e0b";
    ctx.strokeStyle = "#d97706";
    ctx.lineWidth = 4;
    ctx.beginPath();
    ctx.arc(-70, 0, 26, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    // Inner ring hole of key bow
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
    ctx.arc(-70, 0, 10, 0, Math.PI * 2);
    ctx.fill();

    // Key Shank / Shaft (Central backbone extending to the right)
    ctx.fillStyle = "#f59e0b";
    ctx.beginPath();
    ctx.roundRect(-45, -14, 90, 28, 6);
    ctx.fill();
    ctx.stroke();

    // Key Biting Teeth (Protruding exact functional groups matching the receptor lock grooves)
    // Tooth 1 (Top protruding notch)
    ctx.beginPath();
    ctx.roundRect(-15, -42, 24, 30, 5);
    ctx.fillStyle = "#38bdf8";
    ctx.fill();
    ctx.stroke();

    // Tooth 2 (Bottom protruding notch)
    ctx.beginPath();
    ctx.roundRect(-5, 12, 24, 30, 5);
    ctx.fillStyle = "#ec4899";
    ctx.fill();
    ctx.stroke();

    // Tooth 3 (Front tip bitting notch)
    ctx.beginPath();
    ctx.roundRect(35, -20, 25, 40, 6);
    ...
    if (!Number.isNaN(energy) && phase >= 2)
      ctx.fillText(`ΔG = ${energy.toFixed(2)} kcal/mol`, 36, 108);
    ctx.restore();
  }
  ...
  return (
    <div className="w-full max-w-6xl animate-slideUp rounded-[32px] border border-[#b4d2ff]/60 bg-white p-5 sm:p-7 shadow-[0_20px_50px_-10px_rgba(0,40,90,0.12)] relative overflow-hidden">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
          ...
          <div className="relative h-[300px] sm:h-[380px]">
            ...
            <div className="absolute left-4 top-4 z-10 flex gap-5 rounded-full border border-[#d0e0f2] bg-white/95 px-5 py-2 text-[13px] font-medium text-[#1a3652] shadow backdrop-blur">
              ...
              <span className="hidden sm:inline">
                📦{" "}
                <strong className="text-[#0f5bb5]">
                  {STAGES[phaseIdx].label}
                </strong>
              </span>
              <span>
                📉 ΔG{" "}
                <strong className="text-[#0f5bb5]">
                  {Number.isNaN(energy) ? "—" : energy.toFixed(1)}
                </strong>
              </span>
            </div>
            {/* target chip */}
        ...
        <div className="flex flex-col rounded-3xl border border-[#d6e4f5] bg-[#f8fbff] shadow-[0_6px_18px_rgba(0,30,70,0.05)] overflow-hidden">
          ...
          <div className="flex items-start gap-3 border-b border-[#dde9f8] p-4">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <strong className="text-sm text-[#1a3f68]">
                  Docking Agent
                </strong>
              ...
              <p
                key={loadingStage}
                className="agent-bubble mt-1 rounded-xl rounded-tl-sm border border-[#e2ebf6] bg-white p-2.5 text-[13px] leading-snug text-[#1e3450] shadow-sm"
              >
                {narration}
                {loadingStage < 4 && (
          ...
          <div className="flex items-center justify-between px-4 pt-3 text-[13px] font-semibold text-[#1a3f68]">
            ...
            <span className="rounded-full bg-[#e3efff] px-2.5 py-0.5 text-[10px] text-[#1260b0]">
              key decisions
            </span>
          </div>
          <div
            ref={logBoxRef}
            className="flex max-h-[300px] min-h-[180px] flex-1 flex-col gap-2 overflow-y-auto p-3"
          >
            {logs.map((l) => (
          ...
          <div className="flex items-center gap-4 border-t border-[#dde9f8] px-4 py-2.5 text-[11px] text-[#4b6e91]">
            <span>
              <i className="mr-1 inline-block h-2 w-2 rounded-sm bg-[#7c5cbf]" />
              Fetch
            </span>
            <span>
              <i className="mr-1 inline-block h-2 w-2 rounded-sm bg-[#d48f3a]" />
              Bio
            </span>
            <span>
              <i className="mr-1 inline-block h-2 w-2 rounded-sm bg-[#3b8ef0]" />
              Dock
            </span>
            <span>
              <i className="mr-1 inline-block h-2 w-2 rounded-sm bg-[#3b9e6e]" />
              Hit
            </span>
          </div>
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#cfe1fa] bg-[#f3faff] px-5 py-3">
        ...
        <span className="text-[11px] text-[#6a84a5]">
          hands off automatically → 3D viewer · no clicks needed
        </span>
      </div>
    </div>
  );
