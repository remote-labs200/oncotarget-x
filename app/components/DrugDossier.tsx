...
function Row({
  ...
  return (
    <div className="grid grid-cols-[150px_1fr] border-b border-zinc-100 last:border-0">
      <div className="bg-zinc-50/70 px-3 py-2 text-[13px] font-bold text-zinc-700">
        {k}
      </div>
      <div
        className={`px-3 py-2 text-[13px] text-zinc-800 break-words ${mono ? "font-mono" : ""}`}
      >
        {v}
      </div>
    </div>
...
export function DrugDossier({
  ...
  const links = [
    ...
    {
      label: `PubChem · CID ${pub?.pubChemId ?? drug.pubChemId}`,
      href: pubchemUrl(pub?.pubChemId ?? drug.pubChemId),
    },
    chem?.chemblId && {
      label: `ChEMBL · ${chem.chemblId}`,
      href: chemblActivitiesUrl(chem.chemblId),
    },
  ...
  return (
    <div className="animate-fadeIn max-w-6xl mx-auto text-[13px]">
      ...
      <div className="mb-3 flex items-center justify-between gap-2">
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 rounded-lg bg-zinc-100 px-3 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-200 transition cursor-pointer"
        >
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={onPrevDrug}
            className="flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 transition cursor-pointer"
          >
            <ChevronLeft className="h-3.5 w-3.5" />
          </button>
          <span className="font-mono text-[11px] text-zinc-400">
            #{rank}/{target.drugs.length}
          </span>
          <button
            onClick={onNextDrug}
            className="flex h-7 w-7 items-center justify-center rounded-lg bg-zinc-100 text-zinc-600 hover:bg-zinc-200 transition cursor-pointer"
      ...
      <div className="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        ...
        <span className="font-mono text-xs text-zinc-400">
          COMPOUND · RANK #{rank} · {target.name}
        </span>
        <span
          className={`rounded-full px-2 py-0.5 font-mono text-[11px] font-bold ${passes ? "bg-emerald-100 text-emerald-700" : "bg-zinc-100 text-zinc-500"}`}
        >
          {drug.bindingEnergy}
        </span>
      </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <div className="space-y-3 lg:col-span-2">
          <Section title="Identifiers">
            ...
            <Row
              k="DrugBank"
              v={dbUrl ? `${DRUGBANK_IDS[drug.name]} (verified)` : "—"}
            />
          ...
          <Section title={`Structure — 3D stick (${drug.name})`}>
            <div className="h-72 w-full cursor-grab active:cursor-grabbing bg-white">
              {!loading && pub?.sdfUrl ? (
                <LigandSnap
                  sdfUrl={pub.sdfUrl}
                  cid={pub.pubChemId ?? drug.pubChemId}
                  name={drug.name}
                />
              ) : (
            ...
            <p className="border-t border-zinc-100 px-2.5 py-1 font-mono text-[10px] text-zinc-400">
              drag to rotate · scroll to zoom · Jmol colors
            </p>
          </Section>
          ...
          <Section title="ADME properties (PubChem)">
            <Row
              k="Mol. weight"
              v={loading ? "…" : `${pub?.molecularWeight} g/mol`}
            />
            ...
            <Row
              k="Rotatable bonds"
              v={loading ? "…" : String(pub?.rotBonds)}
            />
            <Row k="TPSA" v={loading ? "…" : `${pub?.tpsa} Å²`} />
          </Section>

          <Section
            title={`Lipinski rule of five — ${violations === 0 ? "PASS" : `${violations} violation${violations > 1 ? "s" : ""}`}`}
          >
            {rules.map((r) => (
              <div
                key={r.label}
                className="grid grid-cols-[22px_1fr_auto] items-center gap-2 border-b border-zinc-100 px-3 py-2 last:border-0"
              >
                {r.pass ? (
                ...
                <span className="font-mono text-[13px] font-bold text-zinc-800">
                  {r.value}
                </span>
              </div>
            ))}
          ...
          <Section title="Repurposing record">
            ...
            <Row
              k="Regulatory"
              v={`${drug.fdaStatus} · ${drug.clinicalPhase}`}
              mono={false}
            />
          </Section>

          <Section
            title={`ChEMBL assays ${chem?.chemblId ? `· ${chem.chemblId}` : ""}`}
          >
            {loading ? (
              <p className="px-3 py-3 font-mono text-xs text-zinc-400">
                querying…
              </p>
            ) : (
              (chem?.activities ?? []).map((a: any, j: number) => (
                <div
                  key={j}
                  className="grid grid-cols-4 gap-2 border-b border-zinc-100 px-3 py-2 font-mono text-[13px] last:border-0"
                >
                  <span className="text-zinc-500">
                    {a.assayType === "B" ? "Binding" : "Functional"}
                  </span>
                  <span className="truncate text-zinc-600" title={a.targetName}>
                    {a.targetName}
                  </span>
                  ...
                  <span className="font-bold text-emerald-700">
                    {a.standardValue} {a.standardUnits}
                  </span>
                </div>
              ))
            )}
        ...
        <div className="space-y-3">
          <Section title="Docking scorecard">
            ...
            <Row
              k="Pocket"
              v={`${target.pdbId} · ${target.pathway.split("/")[0].trim()}`}
            />
          </Section>
          ...
          <Section title="Descriptors">
            ...
            <Row
              k="Max phase"
              v={loading ? "…" : `Phase ${chem?.maxPhase ?? "—"}`}
            />
            <Row
              k="Assays"
              v={loading ? "…" : String(chem?.activities?.length ?? 0)}
            />
            ...
            <Row
              ...
              v={
                violations === 0
                  ? "Lipinski PASS"
                  : `${violations} Lipinski violation${violations > 1 ? "s" : ""}`
              }
            />
          </Section>
          ...
          <Section title="Registry pages">
            <div className="space-y-1.5 p-2.5">
              {links.map((l) => (
                ...
                >
                  <span className="truncate font-mono text-xs font-bold text-zinc-700 group-hover:text-cyan-700">
                    {l.label}
                  </span>
                  <ExternalLink className="h-4 w-4 shrink-0 text-zinc-300 group-hover:text-cyan-600" />
                </a>
              ))}
              <p className="px-1 pt-0.5 font-mono text-[11px] leading-snug text-zinc-400">
                exact pages for {drug.name} — visit for full data
              </p>
            </div>
          </Section>

          <Section title="Clinical Trial Matching">
            <div className="p-2.5 rounded-lg border border-zinc-200 bg-zinc-50">
              <p className="text-xs font-mono text-zinc-500 mb-1">
                Active trials for {drug.name}
              </p>
              <div className="space-y-1">
                {drug.clinicalTrials?.map((ct, i) => (
                  <a
                    key={ct.nctId}
                    href={ct.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 rounded bg-zinc-100 px-2 py-1 text-xs font-mono text-zinc-600 hover:bg-zinc-200 hover:text-emerald-600 transition"
                  >
                    <ExternalLink className="h-3.5 w-3.5 text-emerald-500" />
                    <span>{ct.title}</span>
                  </a>
                ))}
                {!(drug.clinicalTrials ?? []).length && (
                  <span className="text-[10px] text-zinc-400">
                    No active ClinicalTrials.gov matches
                  </span>
                )}
              </div>
            </div>
